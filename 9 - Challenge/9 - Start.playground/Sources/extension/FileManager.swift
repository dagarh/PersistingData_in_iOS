...
public extension FileManager {
    ...
    static func getPNGFromDocumentDirectory(subdirectoryName: String, imageName: String) -> UIImage? {
        let imageURL = FileManager.documentDirectoryURL.appendingPathComponent(subdirectoryName, isDirectory: true).appendingPathComponent(imageName).appendingPathExtension("png")
        
        /* It is bringing the image bytes in memory buffer first. Then decoding + de-serialization. UIImage class conforms to the "Codable" protocol. */
